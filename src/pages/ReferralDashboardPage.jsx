 ...
 *
 * Sections:
 *   1. Firm header with Clients | Referrals tab nav
 *   2. Stats bar (pipeline, commissions earned, pending payout)
 *   3. Commission tracker — 3-stage kanban (Pending | Confirmed | Paid)
 *   4. Referral pipeline table
 *   5. "Refer a client" button → copy modal with co-branded intake link
 */
import { useState, useMemo } from 'react'
import { useNavigate }       from 'react-router-dom'
import {
  Building2, DollarSign, Users, TrendingUp, CheckCircle2,
  Clock, Banknote, Link2, Copy, Check, ExternalLink,
  ChevronRight, ArrowRight, Send, BarChart3, Mail,
  MapPin, UserPlus, Layers, AlertTriangle, X, Gift,
  Star, Sparkles,
} from 'lucide-react'
import {
  CPA_FIRM, REFERRAL_CLIENTS, getReferralStats,
  encodeReferralToken, REFERRAL_RATE,
} from '../data/mockData'
import { formatCurrency } from '../lib/utils'

// ─── Shared portal tab nav (reused by CPAPortalPage too) ─────────────────────
...
function ReferClientModal({ open, onClose, token, intakeUrl }) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-5">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">Co-branded intake link for {CPA_FIRM.name}</p>
            </div>
          </div>
        ...
        <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4 space-y-2">
          ...
          <div className="flex items-start gap-2 text-xs text-indigo-700">
            ...
            <span>Your firm name ("Referred by {CPA_FIRM.name}") prominently displayed</span>
          </div>
          <div className="flex items-start gap-2 text-xs text-indigo-700">
            <CheckCircle2 size={12} className="text-indigo-500 mt-0.5 flex-shrink-0" />
            <span>Plain-English SR&ED explainer and typical credit ranges</span>
          </div>
          <div className="flex items-start gap-2 text-xs text-indigo-700">
            <CheckCircle2 size={12} className="text-indigo-500 mt-0.5 flex-shrink-0" />
            <span>"Get your free SR&ED estimate" CTA → TaxLift estimator with ref code attached</span>
          </div>
          <div className="flex items-start gap-2 text-xs text-indigo-700">
            <CheckCircle2 size={12} className="text-indigo-500 mt-0.5 flex-shrink-0" />
            <span>Referral is tracked to your firm automatically — no manual follow-up needed</span>
          </div>
        </div>
...
If your team does any R&D, software development, or solves novel technical problems, you may be eligible for a 35% refundable tax credit on those qualifying expenditures.
...
// ─── Commission tracker ────────────────────────────────────────────────────────
function CommissionTracker({ stats }) {
  const stages = [
    {
      id:    'pending',
      label: 'Pending',
      ...
      credit: stats.pendingCredit,
      commission: REFERRAL_CLIENTS
        .filter(r => r.commission_status === 'pending' && r.commission_cad > 0)
        .reduce((s, r) => s + r.commission_cad, 0),
      description: 'Client scanning or in review — credit estimate TBC',
      clients: REFERRAL_CLIENTS.filter(r => r.commission_status === 'pending'),
    },
    {
      id:    'confirmed',
      label: 'Confirmed',
    ...
    },
    {
      ...
      credit: stats.paidCredit,
      commission: stats.totalCommissionEarned,
      description: 'T661 filed — TaxLift fee collected and commission paid',
      clients: REFERRAL_CLIENTS.filter(r => r.commission_status === 'paid'),
    },
  ]
            ...
            <div className="space-y-2 mb-4">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-semibold text-gray-900">
                  {stage.credit > 0 ? formatCurrency(stage.credit) : '—'}
                </span>
              </div>
              <div className="flex items-center justify-between text-xs">
...
function ReferralTable({ clients }) {
  const sorted = useMemo(
    () => [...clients].sort((a, b) => new Date(b.date_referred) - new Date(a.date_referred)),
    [clients]
  )
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
      ...
      {sorted.map((ref, i) => {
        ...
        const cm     = COMMISSION_META[ref.commission_status] ?? COMMISSION_META.pending
        return (
          <div
            key={ref.id}
            className={`flex flex-col sm:grid sm:grid-cols-[1fr_110px_120px_120px_110px_110px] sm:gap-4 px-5 py-4 hover:bg-gray-50 transition-colors ${!isLast ? 'border-b border-gray-100' : ''}`}
          >
            {/* Company */}
            <div className="flex items-center gap-3 mb-2 sm:mb-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-gray-400 truncate">{ref.industry}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 mb-1 sm:mb-0">
              <StatusPill status={ref.referral_status} meta={STATUS_META} />
            </div>
            ...
            <div className="flex sm:items-center sm:justify-end mb-1 sm:mb-0">
              ...
              <span className="text-sm font-semibold text-gray-900">
                {ref.estimated_credit_cad > 0 ? formatCurrency(ref.estimated_credit_cad) : <span className="text-gray-300 font-normal">TBD</span>}
              </span>
            </div>

            {/* Commission */}
            <div className="flex sm:items-center sm:justify-end mb-1 sm:mb-0 gap-1.5">
              {ref.commission_cad > 0
                ? <>
            ...
            <div className="flex sm:items-center">
              ...
              <span className="text-xs text-gray-500">{formatDate(ref.date_referred)}</span>
            </div>
          </div>
        )
      })}
...
export default function ReferralDashboardPage() {
  ...
  // Build co-branded referral token and intake URL
  const referralToken = encodeReferralToken({
    firmId:      CPA_FIRM.id,
    firmName:    CPA_FIRM.name,
    partnerName: CPA_FIRM.partner_name,
    refCode:     'CROWE2026',
  })
  const intakeUrl = `${typeof window !== 'undefined' ? window.location.origin : 'https://app.taxlift.ai'}/start?ref=${referralToken}`
    ...
    <div className="p-6 max-w-screen-xl mx-auto space-y-6">
      ...
      <div className="bg-gradient-to-r from-slate-900 to-indigo-900 rounded-2xl px-6 py-5">
        <div className="flex items-center justify-between gap-6 flex-wrap">
          ...
          <div className="hidden lg:flex items-center gap-4 text-sm text-slate-300">
            <span className="flex items-center gap-1.5"><Mail size={13} className="text-slate-400" />{CPA_FIRM.partner_email}</span>
            <span className="flex items-center gap-1.5"><MapPin size={13} className="text-slate-400" />{CPA_FIRM.city}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard
          ...
          value={formatCurrency(stats.totalPipelineCredit)}
          sub="Across all referred clients"
          icon={TrendingUp}
        ...
        <StatCard
          ...
          sub="Confirmed, awaiting invoice"
          icon={DollarSign}
          iconColor="text-amber-600"
          iconBg="bg-amber-50"
        />
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Commissions move from Pending → Confirmed (when the package is ready) → Paid (after T661 filing)
            </p>
          </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">{REFERRAL_CLIENTS.length} referred clients · sorted by most recent</p>
          </div>
          <button
            onClick={() => setReferModalOpen(true)}
        ...
        <ReferralTable clients={REFERRAL_CLIENTS} />
      </div>
