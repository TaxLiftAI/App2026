...
// ── Fake blurred T661 narrative ───────────────────────────────────────────────
const FAKE_NARRATIVE = `During the fiscal year, the company undertook a systematic investigation into the feasibility of applying novel gradient checkpointing techniques to reduce GPU memory consumption in the training pipeline. At the outset of the project, it was uncertain whether the selective recomputation approach could maintain training throughput while achieving the target 40% memory reduction. The team formulated the hypothesis that intermediate activations for non-critical layers could be discarded and recomputed on the backward pass without degrading convergence. Multiple experimental iterations were conducted using controlled ablation studies, comparing checkpoint configurations against baseline memory profiles. Initial trials revealed unexpected interaction effects between batch normalization layers and checkpointing boundaries, requiring additional investigation into layer dependency graphs. The systematic resolution of these technical uncertainties constitutes SR&ED activity under ITA Section 248(1).`
...
export default function ScanResultsPage() {
  ...
  const commitCount  = results?.commit_count  ?? clusters.reduce((s, c) => s + (c._commitCount ?? 0), 0)
  ...
  function buildCpaDraft() {
    ...
    const clusterList = topClusters
      .map((c, i) => `  ${i + 1}. ${c._theme ?? c.business_component} — ${fmtK(c.estimated_credit_cad)} estimated ITC`)
      .join('\n')
    return `Hi,
        ...
        <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-2xl p-8 text-white text-center shadow-xl">
          ...
          <div className="mt-6 grid grid-cols-3 gap-3">
            {[
              ...
              { label: 'Eligible commits',    value: commitCount,     Icon: CheckCircle2 },
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          {topCluster && (
            <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4 mb-4">
              ...
              <p className="text-sm font-bold text-indigo-900">{topCluster._theme}</p>
              <p className="text-xs text-indigo-700 mt-0.5">
                {topCluster._commitCount} qualifying commits · {Math.round(topCluster.aggregate_time_hours)} estimated hours
              </p>
          ...
          <div className="space-y-3">
            {topClusters.map((cluster, i) => {
              const theme = cluster._theme ?? 'General R&D'
                ...
                <div key={cluster.id ?? i} className={`rounded-xl border p-4 ${meta.color}`}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-semibold truncate">{cluster.business_component}</p>
                        <p className="text-[11px] opacity-70 mt-0.5">
                          {cluster._commitCount} commits · {Math.round(cluster.aggregate_time_hours)} hrs
                        </p>
                      </div>
                    </div>
        ...
        <div className="relative rounded-2xl overflow-hidden border border-gray-200 shadow-sm">
          ...
          <div className="blur-sm pointer-events-none select-none bg-white p-6">
            <h3 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2">
              <BarChart3 size={15} className="text-indigo-600" /> T661 Narrative Draft — {topCluster?._theme ?? 'ML / AI Development'}
            </h3>
            <p className="text-xs text-gray-600 leading-relaxed">{FAKE_NARRATIVE}</p>
