...
export default function ScanResultsPage() {
  ...
  function handleExportT661Csv() {
    ...
    const rows = [
      ...
      ['Eligible Expenditures (est.)', '', eligibleExp],
      [],
  ...
  // Email gate — show modal if no email captured yet
  const [showEmailGate, setShowEmailGate] = useState(() => {
    try { return !localStorage.getItem('taxlift_scan_email') } catch { return false }
  })
  const [gateEmail,    setGateEmail]    = useState('')
  const [gateTouched,  setGateTouched]  = useState(false)
  const gateEmailValid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(gateEmail)

  function handleEmailGateSubmit(e) {
    e.preventDefault()
    setGateTouched(true)
    if (!gateEmailValid) return
    try { localStorage.setItem('taxlift_scan_email', gateEmail) } catch { /* quota */ }
    setEmail(gateEmail)
    leads.capture({ email: gateEmail, source: 'scan_results_gate' }).catch(() => {})
  ...
  // scanProvince is the province at scan time — read once from sessionStorage (not from the
  // mutable `province` state, which may have been changed by the user on this page).
  const scanProvince   = useMemo(() => {
    try { return sessionStorage.getItem('taxlift_scan_province') ?? 'ON' } catch { return 'ON' }
  }, [])
  const scanProvRate   = PROV_RATES[scanProvince] ?? 0.08
  const scanTotalRate  = 0.35 + scanProvRate   // CCPC + provincial rate used at scan time
  ...
  // eligible expenditure = credit ÷ combined ITC rate (inverts the proxy-method formula)
  // scanTotalRate replaces the old hardcoded 0.43 (which was only correct for ON+CCPC)
  const eligibleExp  = Math.round((baseCredit / scanTotalRate) * (hoursTotal > 0 ? 1 : 0.7))
  ...
  async function handleDownloadPDF() {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        navigate(`/signup?from=scan&plan=starter&credit=${creditAdj}`)
        return
      }
      if (!res.ok) {
...
Generated by TaxLift (taxlift.ai) · Free SR\u0026ED scan`
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {/* ── Email gate modal ── shown once if no email captured yet */}
      {showEmailGate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-8">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-gray-500 mt-1">Enter your work email to see your SR&amp;ED estimate.</p>
            </div>
            <form onSubmit={handleEmailGateSubmit} className="space-y-3">
              <input
                type="email" autoFocus required
                placeholder="you@yourcompany.com"
                value={gateEmail}
                onChange={e => setGateEmail(e.target.value)}
                className={`w-full border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/30 ${gateTouched && !gateEmailValid ? 'border-red-400' : 'border-gray-200'}`}
              />
              {gateTouched && !gateEmailValid && (
                <p className="text-xs text-red-500">Please enter a valid email address.</p>
              )}
              <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 rounded-xl text-sm transition-colors">
                See my SR&amp;ED estimate →
              </button>
              <p className="text-center text-[11px] text-gray-400">No spam. We'll email you your full report.</p>
            </form>
          </div>
        </div>
      )}
      ...
      <nav className="bg-white border-b border-gray-200 sticky top-0 z-20">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <TaxLiftLogo size="sm" variant="light" />
            {isDemoMode && (
            ...
            <span className="hidden sm:inline-flex items-center gap-1 text-[10px] font-semibold bg-red-50 text-red-700 border border-red-200 rounded-full px-2.5 py-0.5 tracking-wide">
              🍁 Proudly Canadian
            </span>
          </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-2xl p-5 sm:p-8 text-white text-center shadow-xl">
          ...
          <div className="flex flex-wrap items-baseline justify-center gap-x-2 gap-y-0 mb-2">
            <span className="text-4xl sm:text-5xl lg:text-6xl font-bold tabular-nums tracking-tight">
              {fmtK(animLow)}
            </span>
          ...
          {showMethodology && (
            <div className="border-t border-gray-100 px-5 py-4 bg-gray-50 text-xs text-gray-600 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <p className="font-semibold text-gray-800 mb-1.5">CRA proxy method inputs</p>
                  <div className="space-y-1">
                    <div className="flex justify-between"><span>Team size (R&amp;D)</span><span className="font-mono">{results?.team_size ?? '—'} employees</span></div>
                    <div className="flex justify-between"><span>Avg T4 salary</span><span className="font-mono">$120K</span></div>
                    <div className="flex justify-between"><span>SR&amp;ED eligibility</span><span className="font-mono">{results?.eligibility_pct ?? '—'}%</span></div>
                    <div className="flex justify-between"><span>Overhead proxy (T4088)</span><span className="font-mono">55%</span></div>
                  </div>
                </div>
              </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          {!isPaid && (
            <div className="border-t border-gray-100 px-5 py-4 bg-gray-50">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3 opacity-50 pointer-events-none select-none">
                {[
        ...
        <div className="flex items-center justify-between gap-3 bg-white rounded-2xl shadow-sm border border-indigo-100 px-5 py-3.5">
          ...
          <button
            onClick={() => isPaid ? handleDownloadPDF() : navigate(`/signup?from=scan&plan=starter&credit=${creditAdj}`)}
            disabled={pdfBusy}
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100">
            <p className="text-sm font-semibold text-gray-900">Your estimate is ready — here's what happens next</p>
            <p className="text-xs text-gray-400 mt-0.5">Free account to save your scan · $999 flat fee to file</p>
          </div>
          <div className="grid sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-gray-100">
            ...
            <div className="px-5 py-4">
              ...
              <ul className="space-y-2">
                {[
                  ...
                  'Save & revisit your scan results',
                ].map(item => (
                ...
                {[
                  'T661 narrative drafts',
                  'CPA-ready handoff PDF',
                  'Audit vault & evidence chain',
                ].map(item => (
            ...
            <div className="px-5 py-4 bg-indigo-50/40">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs font-bold text-indigo-700 uppercase tracking-wide">SR&amp;ED Filing Package</span>
                <span className="text-[10px] bg-indigo-100 text-indigo-700 border border-indigo-200 rounded-full px-2 py-0.5 font-medium">$999 flat fee</span>
              </div>
              <p className="text-[10px] text-indigo-400 mb-3">No % of your refund taken — keep every dollar CRA sends</p>
              <ul className="space-y-2">
                {[
                  'AI T661 narrative drafts per project',
                  'CPA-ready handoff package PDF',
                  'CPA review portal — share with your firm',
                  'SHA-256 commit evidence chain',
                  '18-month lookback & multi-year claims',
                  '3-year audit vault included',
                ].map(item => (
              ...
              <button
                ...
                className="mt-4 w-full flex items-center justify-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold py-2.5 rounded-lg transition-colors"
              >
                Get started — $999 <ArrowRight size={11} />
              </button>
              <p className="text-[10px] text-center text-gray-400 mt-2">One-time · no subscription · 1 fiscal year</p>
            </div>
          </div>
        </div>
